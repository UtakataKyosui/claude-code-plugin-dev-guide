...
補助スクリプトでJSON解析・入力検証・API連携などを実装する場合は、Python＋uvを優先候補とします。標準ライブラリで足りるなら依存を増やさず、単純なコマンドの連結にはShellも使います。[言語の選択とuvの管理例](hooks.md#スクリプトの言語と依存管理を選ぶ)に従い、本文にはスクリプトの入力・出力・失敗時の扱いを記載してください。
