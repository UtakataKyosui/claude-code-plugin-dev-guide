...
本ガイドでは、JSON解析、入力検証、API連携、複雑な分岐・例外処理を伴うスクリプトはPythonを優先します。速度だけでなく、読みやすさ、テストのしやすさ、利用者の実行環境で選びます。既存の言語・実装が適切なら、言語の統一だけを目的に書き換えません。この方針はHookだけでなく、SkillやSubagentから使う補助スクリプトにも適用します。

| 用途 | 推奨する構成 |
| --- | --- |
| 数個のコマンドを順番に実行する | Shell |
| 単体の検査・変換・API連携 | Python＋uvのスクリプト管理 |
| 複数モジュール・共通処理・まとまったテスト | Python＋`pyproject.toml`＋`uv.lock` |

まず`json`、`pathlib`、`argparse`などの標準ライブラリで足りるか検討します。外部依存は必要なものだけ宣言し、ロックファイルを配布物に含めます。uvで依存関係を管理しても、利用者へのuv・Python・ネットワーク要件の説明は必要です。

### 単体Pythonスクリプトの管理例

以下は開発時の例で、`validate_plugin.py`は自分で実装するスクリプトです。`pyyaml`はYAML解析が必要な場合のみ追加します。

```sh
uv init --script validate_plugin.py --python 3.12
uv add --script validate_plugin.py pyyaml
uv lock --script validate_plugin.py
uv run --locked validate_plugin.py
```

依存関係はPythonファイル先頭のインラインメタデータに記録され、スクリプト用のロックは`validate_plugin.py.lock`に保存されます。標準ライブラリだけの場合は`dependencies = []`とし、パッケージ追加を省略します。インラインメタデータを使うスクリプトは、実行先プロジェクトの依存環境から独立して扱われます。出典: [uvのスクリプト管理](https://docs.astral.sh/uv/guides/scripts/)。

複数モジュールのツールは`pyproject.toml`と`uv.lock`で管理し、準備時に`uv sync --locked`を使います。`--locked`はロックの不整合を検出する指定で、パッケージ取得を禁止する指定ではありません。出典: [uvのロックと同期](https://docs.astral.sh/uv/concepts/projects/sync/)。

### Hookの発火前に実行環境を準備する

同期Hookの実行時に初めてPythonや依存パッケージを取得すると、待ち時間やネットワーク障害が通常作業に影響します。導入手順で環境を準備し、安全な自己診断などで依存がそろうことを確認してください。副作用のある本処理を、準備のためだけに実行しないようにします。

単体スクリプトの場合、準備済みの環境では次のようにネットワーク取得を禁止して実行できます。

```sh
uv run --offline --locked validate_plugin.py
```

`--offline`はスクリプト自身のネットワーク通信を禁止するものではありません。必要なPython・依存が未準備なら失敗するため、キャッシュがない状態と準備後の状態を別々に検証します。仮想環境やuvのキャッシュはソース管理せず、Pluginの更新で置き換わる配布ディレクトリへ実行時の環境を書き込む設計も避けます。

PythonからHookを実装する場合も、標準入力のJSON、標準出力・標準エラー、イベント別の終了コードの契約は変わりません。例外で終了コード1になっただけでは保護対象をブロックできない場合があるため、検査不能時の扱いを明示します。
